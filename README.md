...
The key rotation allows the user to rotate the keys of an account. The new keys are generated by the client and sent to the server, which adds the new keys to the account and marks the old keys as revoked. The old keys are deleted if they are revoked and not used in any other transfer, which ensures that old transfers can still be accessed with the old keys, even after the key rotation.
